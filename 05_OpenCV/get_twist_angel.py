...
def parse_args():
    parser = argparse.ArgumentParser(description="opencv predict MoS2 twist angle")
    # exclude background
    ...
    parser.add_argument("--result-image", default=False, type=bool,
                        help="draw label in origin image or segmentation image")
    parser.add_argument("--result-type", default=True, type=bool, help="optional result image type")
    parser.add_argument("--get-layers", default=False, type=bool, help="get every layers of predict result image")
    parser.add_argument("--draw-interior", default=False, type=bool,
                        help="Draw the internal angle of triangle to the result diagram")
    parser.add_argument("--predict-all-triangle", default=True, type=bool,
                        help="Whether to detect all triangles or torsional angle information")
    args = parser.parse_args()
    return args


# 获取最左侧两个点,输入的为三角形的点集合
...
def get_twist_angle(args):
    ...
    mask_path = 'test_result.png'
    img = cv2.imread(args.image_path, cv2.IMREAD_UNCHANGED)
    img2 = cv2.imread(mask_path, cv2.IMREAD_UNCHANGED)
    ...
    inrange_color = [color_1, color_2, color_3]
    ...
    for co in inrange_color:
        ...
        if args.get_layers:
            ...
            cv2.imwrite('layers_{}.png'.format(i + 1), roi_mask_array)  # 保存每层识别的结果图
        bbox_list = []
        # 使用轮廓寻找函数来获取最外层轮廓
            ...
            if min_area < cv2.contourArea(c):
                ...
                if args.result_type:
                    ...
                    if corners == 3 or corners == 4:
                        # 使用最小外接三角形函数得到拟合三角形的面积以及三个顶点位置坐标
                        ...
                        # 如果想要结果图像输出所有拟合三角并绘制旋转角度，就选择True，否则就检测扭角信息
                        ...
                        if args.predict_all_triangle:
                            if min_angle < interior_a and min_angle < interior_b \
                                ...
                                cv2.line(result_img, (int(point_triangle1[0][0]), int(point_triangle1[0][1])),
                                         (int(point_triangle1[1][0]), int(point_triangle1[1][1])), (200, 200, 255), 2,
                                         cv2.LINE_8)
                                cv2.line(result_img, (int(point_triangle1[0][0]), int(point_triangle1[0][1])),
                                         (int(point_triangle1[2][0]), int(point_triangle1[2][1])), (200, 200, 255), 2,
                                         cv2.LINE_8)
                        ...
                        # get twist angle
                            ...
                            if (((np.array(test_list))[0][0].all() == (np.array(color_1))[0][0].all()) and (
                                ...
                                if min_angle < interior_a and min_angle < interior_b \
                                    ...
                                    # 获取MoS2拟合三角形最左侧的两个点,并得到1L相对于图像中心的旋转角度
                                    ...
                                    for ci in contours1:
                                        ...
                                        if min_area < cv2.contourArea(ci):
                                            ...
                                            # 通过arcLength函数获取轮廓的周长，传入多边形拟合函数中以得到轮廓角点信息
